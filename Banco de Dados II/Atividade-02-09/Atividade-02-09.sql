DROP DATABASE IF EXISTS Aula_join;
...
SELECT Nome, dNome FROM Funcionario f JOIN Departamento d ON f.idDepartamento = d.idDepartamento;
...
SELECT Nome, Sobrenome FROM Funcionario f JOIN Departamento d ON f.idDepartamento = d.idDepartamento WHERE Orcamento > 60000;
-- 4) Liste os nomes de todos os departamentos que possuam mais de dois funcionários
SELECT dNome FROM Departamento d JOIN Funcionario f ON f.idDepartamento = d.idDepartamento GROUP BY d.idDepartamento HAVING COUNT(Nome) > 2; 
